import torch
import torch.nn as nn 
import torch.nn.functional as F 

from einops import rearrange, repeat
from einops.layers.torch import Rearrange


class PreNorm(nn.Module):
    def __init__(self, dim, fn):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        self.fn = fn
    def forward(self, x, **kwargs):
        return self.fn(self.norm(x), **kwargs)

class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout=0.):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(dim, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, dim),
            nn.Dropout(dropout)
        )
    def forward(self, x):
        return self.net(x)
    
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        super().__init__()
        inner_dim = dim_head * heads 
        self.heads = heads 
        self.scale = dim_head ** (-0.5)
        self.attend = nn.Softmax(dim=-1)
        self.dropout = nn.Dropout(dropout)

        #self.query = nn.Linear(dim, inner_dim, bias=False)
        #self.key = nn.Linear(dim, inner_dim, bias=False)
        #self.value = nn.Linear(dim, inner_dim, bias=False)

        self.to_qkv = nn.Linear(dim, inner_dim * 3, bias = False)

        self.to_out = nn.Sequential(
            nn.Linear(inner_dim, dim),
            nn.Dropout(dropout)
        )
    def forward(self, x): 
        qkv = self.to_qkv(x).chunk(3, dim = -1)
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = self.heads), qkv)
        dots = torch.matmul(q, k.transpose(-1, -2)) * self.scale

        attn = self.attend(dots)
        attn = self.dropout(attn)

        out = torch.matmul(attn, v)
        out = rearrange(out, 'b h n d -> b n (h d)')
        return self.to_out(out)
    
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, out_dim, dropout=0.):
        super().__init__() 
        self.layers = nn.ModuleList([])
        for _ in range (depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention(dim=dim, heads=heads, dim_head=dim_head, dropout=dropout)),
                PreNorm(dim, FeedForward(dim, mlp_dim, dropout))
            ]))


        self.feature_token = nn.Parameter(torch.randn(1, 1, dim))

        self.mlp_head = nn.Sequential( 
            nn.LayerNorm(dim),
            nn.Linear(dim, out_dim)
        )
    def forward(self, x):
        b, n, _, = x.shape
        feature_token = repeat(self.feature_token, '1 1 d -> b 1 d', b = b)
        x = torch.cat((feature_token, x), dim=1)
        print(x.shape)
        for attn, ff in self.layers:
            x = attn(x) + x
            x = ff(x) + x
        out_feature = x[:, 0]
        out = self.mlp_head(out_feature)
        return out
    


def main(): 
    
    Network = Transformer(dim=128, depth=2, heads=2, dim_head=64, mlp_dim=128, out_dim=128, dropout=0).cuda()
    feature = torch.rand((43200, 7, 128)).cuda()
    out = Network(feature)
    print(Network)
    print(out.shape)
    input("...")


    pass


if __name__=='__main__': 
    main()